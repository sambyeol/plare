...
class Namespace:
  ...
  def __repr__(self):
    return 'Namespace({})'.format(', '.join(['{}={}'.format(k, v) for k, v in self.data.items()]))
...
class Lexer:
  ...
  def __init__(self, tokens, ignore=None, comments=None):
    ...
    self.__ignore_regex = {t: re.compile(r) for t, r in ignore.items()} if ignore else {}
    self.__comment_regex = [(re.compile(start), re.compile(end)) for start, end in comments] if comments else {}
