...
from plare.lexer.lexer import EOF, EPSILON
from plare.lexer.token import Token
from plare.parser.automata import Edge, EdgeSet, State, StateSet
from plare.parser.exception import ReduceReduceConflict, ShiftReduceConflict
from plare.parser.rule import ARule, ARuleSet, Constructor, Rule
...
class Parser:
    ...
    def __init__(
        ...
        verbose=False,
        precedence=None,
        right_associative=None,
    ):
        self.__lexer = lexer

        precedence = {
            t: precedence[t] if t in precedence.keys() else 0
            for t in self.__lexer.TOKENS.to_list()
        }
        right_associative = (
            right_associative
            if type(right_associative) == list
            else [right_associative] if right_associative else []
        )
        ...
        def first(string):
            ...
            while look_next:
                ...
                if isinstance(s, Token):
                    ...
                    raise TypeError(
                        "[Error] Parser first: Unknown symbol {} is given.".format(s)
                    )
            return first_set
        ...
        for edge in E:
            if verbose:
                print("\rMaking parser...{}".format(next(windmill)), end="")
            self.__table[edge.I.name, edge.X] = (
                "shift" if isinstance(edge.X, Token) else "goto",
                edge.J.name,
            )
                ...
                if arule.is_ended and arule.n is not None:
                    for Y in arule.lookahead:
                        ...
                        try:
                            ...
                            p_y = precedence[Y]
                            if p_r > p_y:
                                self.__table.resolve_conflict(
                                    I.name, Y, ("reduce", arule.n)
                                )
                            elif p_r < p_y:
                                pass
                            else:
                            ...
                            if p_o > p_n:
                                ...
                                raise ReduceReduceConflict(
                                    "[Parser] __init__: Cannot resolve conflict between two reduce rules.\n\t{}\n\t{}".format(
                                        _arule, arule
                                    )
                                )
        if verbose:
