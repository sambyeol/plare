...
class State:
  ...
  def __repr__(self):
    return '---{}---\n'.format(self.__name) + '\n'.join([str(r) for r in self.__arules])
  ...
  def merge(self, other, return_update=False):
    ...
    for arule in other:
      i = self.find_idx(arule)
      self.__arules[i], u = self.__arules[i].add_lookahead(arule.lookahead, return_update=True)
...
class StateSet:
  ...
  def __init__(self, states=None):
    self.__states = [s.set_name('s{}'.format(i)) for i, s in enumerate(states)] if states else []
  ...
  def add(self, state, return_updated=False):
    ...
    if i < 0:
      ...
      self.__states[i], updated = self.__states[i].merge(state, return_update=True)
    return (self, self.__states[i], updated) if return_updated else (self, self.__states[i])
...
class Edge:
  ...
  def __eq__(self, other):
    ...
    if self.__to != other.J:
      raise DoublePointingEdge('[Error] Edge {}: New edge comes that points {}.'.format(self, other.J.name))
    return True
