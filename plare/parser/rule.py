...
class Constructor:
    ...
    def __init__(self, constructor, args):
        self.__const = (
            constructor if constructor else (lambda *x: (x[0] if len(x) == 1 else x))
        )
        self.__args = [
            int(i[1:]) - 1 if type(i) == str and i.startswith("$") else i for i in args
        ]

    def __call__(self, xs):
        args = [
            (
                (xs[i].value if isinstance(xs[i], TokenObject) else xs[i])
                if type(i) == int
                else i
            )
            ...
            for rule, _ in self.__rules:
                ...
                while look_next:
                    ...
                    if isinstance(s, Token):
                        ...
                        raise TypeError(
                            "[Error] Rule {}: Unknown symbol {} is given.".format(
                                self.__name, s
                            )
                        )
...
class ARule:
    ...
    def next(self, variable):
        return (
            ARule(
                ...
                loc=self.__loc + 1,
                n=self.__n,
            ).add_lookahead(self.__lookahead)
            if self.__loc < len(self.__right) and variable == self.__right[self.__loc]
            else None
        )
    ...
    def __repr__(self):
        ...
        return (
            self.__left
            + " -> "
            + " ".join(right[: self.__loc] + ["."] + right[self.__loc :])
            + " | "
            + str(self.__lookahead)
        )
...
class ARuleSet:
    ...
    def startswith(self, variable):
        ...
        for arule in self.__arules:
            if (
                isinstance(variable, Rule)
                and arule.left == variable.name
                and arule not in rules
            ):
                rules.append(
                    ARule(arule.left, arule.right, arule.constructor, n=arule.n)
                )
        return rules
