...
class Type:
    ...
    def __init__(self, name, alias=None, **kwargs):
        ...
        if self.__alias and len(kwargs) > 0:
            print(
                "[Warning] Type {}: Other arguments will be discarded in alias mode".format(
                    self.__name
                )
            )
        self.__dict = (
            ...
            if self.__alias
                ...
                k: t if t is None or type(t) == list else [t] for k, t in kwargs.items()
            }
        )
    ...
    def build(self, types):
        ...
        if self.__alias:
            self.__alias = [
                (t[:-1], t[-1]) if type(t) == str and t[-1] in "?*" else (t, None)
                for t in self.__alias
            ]
            self.__alias = [
                (types[t] if type(t) == str else t, o) for t, o in self.__alias
            ]
    ...
    def __repr__(self):
        if self.__alias:
            return "type {} = {}".format(
                ...
                " * ".join(
                    [
                        (t.name if isinstance(t, Type) else t.__name__)
                        + (" list" if o == "*" else " option" if o == "?" else "")
                        for t, o in self.__alias
                    ]
                ),
            )
        else:
    ...
    def __call__(self, *values):
        ...
        if self.__dict:
            raise TypeError(
                "[Error] Type {}: Non-alias type cannot be a constructor.".format(
                    self.__name
                )
            )
        ...
        if len(values) > len(self.__alias):
            raise TypeError(
                "[Error] Type {}: This type takes at most {} arguments, but {} arguments are given.".format(
                    self.__name, len(self.__alias), len(values)
                )
            )

        if values not in self:
            raise TypeError(
                '[Error] Type {}: Type mismatch. Expecting "({})".'.format(
                    self.__name,
                    ", ".join(
                        [
                            t.name if isinstance(t, Type) else t.__name__
    ...
    def __contains__(self, elem):
        ...
        if self.__alias:
            if (type(elem) != list and type(elem) != tuple) or (
                len(self.__alias) == 1
                and self.__alias[0][1] == "*"
                and elem[0] in self.__alias[0][0]
            ):
                elem = [elem]
            for el, t_o in zip(elem, self.__alias):
                ...
                if o == "?":
                    if not (
                        el is None
                        or (el in t if isinstance(t, Type) else type(el) == t)
                    ):
                        return False
