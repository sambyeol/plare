...
from plare import Lexer, Parser, Rule, Token, Type, build_grammar, build_lang
...
def main(args):
    ...
    for calc_file in args.file:
        ...
        if not calc_file.endswith(".calc"):
            raise Exception(
                'File extension is not ".calc". Please check the argument "{}".'.format(
                    calc_file
                )
            )
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="File that have integer arithmetic expressions",
    )
